...
Nur falls dieser Timeout ausläuft, bevor \lstinline$set()$ aufgerufen wurde, gibt \lstinline$False$ zurück.
...
Diese drei Klassen nutzen intern \lstinline$Lock$- Objekte, weshalb Threads bei gleichzeitigen zugriffen
blockiert werden.
...
Durch angebe des optionalen Parameters \lstinline$timeout$ kann eine maximale Zeit angegeben werden,
...
Um eine Element aus der Queue heraus zu holen, wird \lstinline$get()$ aufgerufen.
Sollte zum Zeitpunkt des Aufrufs keine Element in der Queue enthalten sein, wird so lange blockiert,
bei ein anderer Thread eines hinzufügt.
...
Neben dieser grundlegenden Funktionalität für eine Queue implementieren die drei vorgestellten 
...
Die beiden optionalen Parameter \lstinline$block$ und \lstinline$timeout$ werden ignoriert und werden 
...
Die beiden Methoden \lstinline$task_done()$ und \lstinline$join()$ werden von \lstinline$SimpleQueue$ 
nicht angeboten.
