...
Um Daten zwischen Prozessen auszutauschen bietet Python die \lstinline$Queue$-Klasse und die
\lstinline$Pipe$-Funktion an.
Diese \lstinline$Queue$ aus dem \lstinline$multiprocessing$-Modul ist der bereits bekannten
...
Wenn noch keine Objekt gesendet wurde, so blockiert \lstinline$recv()$ bis ein Objekt zum Empfang 
bereit ist.
...
Es ist möglich, einen optionalen Timeout anzugeben, blockiert \lstinline$poll()$ maximal die spezifizierte
Zeit in Sekund.
...
Durch \lstinline$recv_bytes()$ kann ein dieses Objekt empfangen werden.
