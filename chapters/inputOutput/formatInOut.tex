...
Es wäre sicherlich hilfreich, wenn wir die String-Ausgaben nach belieben formatieren könnten. Bisher haben wir den Seperator von \emph{print()} kennengelernt - dieser ist von seiner Funktionsweise jedoch stark beschränkt. Python bietet uns hierfür die \emph{format()}-Methode an, vorher betrachten wir aber die Modulo-Arithmetik und machen uns mit den Formatierungszeichen vertraut.
...
%[Flag][Minimum der Gesamtlänge].[Präzision][Typ]
 ...
 Mittels Punkt können wir folgend die Präzision einstellen, was bei einem \emph{Float}-Datentyp die Nachkommastellen bestimmt. Alle Zahlen werden zu der angegebenen Nachkommastelle aufgerundet!
...
Python bietet uns für die Formatierung von String-Elementen die Methode \emph{format()}.
...
\emph{format()} ersetzt markierte Stellen im gegebenen String durch angegebene Werte (Parameter in \emph{format()}) und liefert diesen zurück.
...
Python bietet uns für \lstinline{Dictionaries} einen einfachen Weg, diese mittels \emph{format()} und der Nutzung von Schlüsselwort-Parametern, auszugeben. 
